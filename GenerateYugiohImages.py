...
CARD_DIR = "./YugiohPNGImages"
EFFECTS_DIR = "./Effects"
NUM_IMAGES = 40000

# Output directories
BASE_DIR = "./YugiohModel"
OUTPUT_DIRS = {
    "train": {
...
def random_transform(card_img, num_cards, output_size=(480, 640)):
    ...
    if num_cards == 1:
        ...
        scale = np.random.uniform(1/(num_cards*5)+.1, 2/(num_cards*5)+.1)
...
for i in range(int(NUM_IMAGES * .8)):
    ...
    for x in range(num_cards):
        ...
        """if random.random() < 0.25:
            # Load glare image
            glare_path = os.path.join(EFFECTS_DIR, "Glare")
            glare_file = random.choice(os.listdir(glare_path))
            glare_img = Image.open(os.path.join(glare_path, glare_file)).convert("RGBA")
